...
# 添加模型所在目录到Python路径
sys.path.append('/mnt/data3/XIN/model/MambaMIL-main/')
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        # Follow MambaContMix forward pass structure
        h = features
        
        # 特征降维
        h = model._fc1(h)  # [B, n, 512]
        
        # 主路径：Mamba序列建模
        mamba_out = h
        if model.type == "SRMamba":
            for layer in model.layers:
                h_ = mamba_out
                mamba_out = layer[0](mamba_out)
                mamba_out = layer[1](mamba_out, rate=model.rate)
                mamba_out = mamba_out + h_
        elif model.type == "Mamba" or model.type == "BiMamba":
            for layer in model.layers:
                h_ = mamba_out
                mamba_out = layer[0](mamba_out)
                mamba_out = layer[1](mamba_out)
                mamba_out = mamba_out + h_
        
        # 辅助路径：ContMix 2D特征建模
        contmix_in = model.unsqueeze_layer(h)  # [B, 512, spatial_size, spatial_size]
        contmix_in = model.contmix_norm(contmix_in)
        contmix_out = model.contmix_block(contmix_in)  # [B, 512, spatial_size, spatial_size]
        contmix_out = model.squeeze_layer(contmix_out)  # [B, spatial_size², 512]
        
        # 处理序列长度不匹配的情况
        if contmix_out.shape[1] != mamba_out.shape[1]:
            contmix_out = F.interpolate(
                contmix_out.permute(0, 2, 1).unsqueeze(-1),
                size=(mamba_out.shape[1], 1),
                mode='bilinear',
                align_corners=False
            ).squeeze(-1).permute(0, 2, 1)
        
        # 路径融合
        gate = model.fusion_gate(mamba_out)
        fused_out = gate * mamba_out + (1 - gate) * contmix_out
        
        # 应用注意力机制
        fused_out = model.norm(fused_out)
        A = model.attention(fused_out)  # [B, n, K]
        A = torch.transpose(A, 1, 2)
        A = F.softmax(A, dim=-1)  # [B, K, n]
        h = torch.bmm(A, fused_out)  # [B, K, 512]
        h = h.squeeze(0)
        
        # 分类器
        logits = model.classifier(h)  # [B, n_classes]
        
        # 计算中心点响应（用于梯度计算）
        central_point = torch.nn.functional.leaky_relu(logits).sum()
        ...
        grad = grad.reshape(1, -1, 32, 32)  # 调整为合适的形状
        ...
        if meter.count == len(process_stack):
            np.save('/mnt/data3/heatmap/erf/temp1', meter.avg)
