...
if __name__ == '__main__':
    ...
    model = torch.load(model_path.format('int'), map_location=torch.device('cuda'),weights_only=False)
    ...
    if n_samples <= 1:
        raise ValueError(f"Not enough samples for t-SNE. Found {n_samples} sample(s), but t-SNE requires at least 2 samples.")
    
    # Adjust perplexity based on the number of samples (default is 30, must be less than n_samples)
